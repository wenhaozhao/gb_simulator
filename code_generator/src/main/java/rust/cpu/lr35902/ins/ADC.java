...
public class ADC implements Ins {
    @Override
    public String fnExec(Opcode opcode) {
        ...
        return STR. """
                fn exec(&self, cpu: &mut LR35902) -> u8 {
                        ...
                        let flag_c = cpu.register.get_flag(Flag::C) as u8;
                        let res = left.wrapping_add(right).wrapping_add(flag_c);
                        cpu.register.set_flag(Flag::Z, res == 0x00);
                        cpu.register.set_flag(Flag::N, false);
                        cpu.register.set_flag(Flag::H, (left & 0x0F) + (right & 0x0F) + (flag_c & 0x0F) > 0x0F);
                        cpu.register.set_flag(Flag::C, ((left as u16) & 0x00FF) + ((right as u16) & 0x00FF) + ((flag_c as u16) & 0x00FF) > 0x00FF);
